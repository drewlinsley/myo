...
def process_video(mask, vid, file_name, ss, remove_npy_pattern, smallest_spark, longest_spark, max_area, num_frames, plot_videos, annotated_video_dir):
    """
    Process a single video to detect and analyze calcium sparks.

    Args:
        mask (np.ndarray): Binary mask of detected sparks.
        vid (np.ndarray): Video data.
        file_name (str): Name of the video file.
        ss (float): Spark score.
        remove_npy_pattern (str): Regex pattern to remove from filenames.
        smallest_spark (int): Minimum size of a spark to be considered.
        longest_spark (int): Maximum allowed duration of a spark.
        max_area (int): Maximum area of a spark.
        num_frames (int): Number of frames to process.
        plot_videos (bool): Whether to create annotated videos.
        annotated_video_dir (str): Directory to save annotated videos.

    Returns:
        tuple: Video statistics and cell statistics.
    """
    vid = vid.squeeze()
    ...
    cell_stats = pd.DataFrame.from_dict(video_stats).groupby(["file_name", "experimental_condition", "experiment_name", "perturbation"]).mean().reset_index()
...
def main(
    ...
    for source in tqdm(test_loader, total=len(test_loader)):
        ...
        # Parallelize the processing of videos
        results = joblib.Parallel(n_jobs=-1)(
            joblib.delayed(process_video)(
                mask, vid, file_name, ss, remove_npy_pattern, smallest_spark,
                longest_spark, max_area, num_frames, plot_videos, annotated_video_dir
            )
            for mask, vid, file_name, ss in zip(masks, whole, fn, sss)
        )

        # Collect results
        for video_stat, cell_stat in results:
            all_stats.extend(video_stat)
            cell_stats.append(cell_stat)
