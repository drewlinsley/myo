...
def main(config):
    """
    Preprocess calcium imaging videos.

    This function reads raw .nd2 video files, processes them by smoothing and rescaling,
    ...
    fns = glob(os.path.join(raw_videos, "*Experiment*", "*", "*.nd2"))
    ...
    for fn in fns:
        ...
        out_numpy_path = "{}.npy".format(processed_npys + fn.replace(raw_videos, ""))
        out_video_path = "{}.mp4".format(processed_videos + fn.replace(raw_videos, ""))
        ...
        # Load data
        arr = nd2.imread(vid_file, xarray=True)
        narr = arr.to_numpy()  # .astype(np.float32)

        # Transpose to fix contiguous array issue
        narr = np.ascontiguousarray(narr.transpose(1, 0, 2))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Preprocess calcium imaging videos.")
