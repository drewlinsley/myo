...
class UnetDecoder(nn.Module):
    ...
    def __init__(
        ...
        if extra_scale > 0:
            self.blocks.append(DecoderBlock(out_chs, out_chs, norm_layer=norm_layer))
        # self.final_conv = []
    ...
    def forward(self, x: List[torch.Tensor]):
        ...
        #     x = F.interpolate(x, scale_factor=self.extra_scale, mode='nearest').contiguous()
        if not self.no_dapi:
